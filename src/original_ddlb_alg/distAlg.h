...
class DistAlg : public omnetpp::cSimpleModule
{
  private:
  ...
  public:
    ...
    void resets(uint&flag,double&sum_local_loadcosts,double&sumvolts);
    ...
    void initializeBusParameters(double *bus, double *powers, double *rpowers,
        uint slack_bus_ID, double &genSum, double &loadSum);
...
double powers[BUSNUM],	rpowers[BUSNUM],
       mat_powers_c[BUSNUM],real_volts[BUSNUM],loadCost[BUSNUM];
...
int DistAlg::getBusvals(const std::string& input){
  if(!_getBusValsDone){
    ...
    slack_bus_ID = findnoofGensAndSlack(BUSNUM,slack); //find slack ID
...
void DistAlg::loadFlowSolution()
{
  ...
  matlabrun(bus,linefull,no_Of_Gens,slack_bus_ID+1
     ,no_Of_slack,bus_sol);
  ++matlabCallCount;
  ...
  if(find (printRounds.begin(), printRounds.end(), matlabCallCount) != printRounds.end()){
...
void DistAlg::singleRunLoadFlow()
{
  ...
  initializeBusParameters(bus,asked_power,rpowers,
      slack_bus_ID,M,L);
  iter = matlabrun(bus,linefull,no_Of_Gens,slack_bus_ID+1
      ,no_Of_slack,bus_sol);

  single_run_slack = 	bus_sol[BUSNUM*5+slack_bus_ID] - bus_sol[BUSNUM*3+slack_bus_ID];
